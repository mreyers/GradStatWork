...
# The MSPE values for this model and onwards are wrong, except for the baseline (1.41) and
# the gbm_rf ensemble (1.339)
...
# This basic 2 model ensemble runs a MSPE of 1.53 and a RMSE of 1.23 on the test set.
# How much better is it than the individual models?
perf_gbm_test <- h2o.performance(base_gbm, newdata = test_h2o) # 1.58
perf_rf_test <- h2o.performance(base_rf, newdata = test_h2o) # 1.57
...
# Eval ensemble performance on a test set
perf <- h2o.performance(ensemble, newdata = test_h2o) # With random discrete, n =10, get mspe = 1.50
# Same results with cartesian on 100 trees
