...
covMatrixCalcBasic <- function(fullX, fullY){
  ...
  for(i in 1:nrow){
    for(j in 1:ncol){
      covMatrix[i, j] <- covCalc(x[i], x[j])
    }
  }
  return(covMatrix)
}

myResult <- covMatrixCalc(x, y)
myResult

covMatrixGaussian <- function(fullX, fullY, theta){
...
covMatrixG <- covMatrixGaussian(x, y, theta = 2)
```
...
# Other theta test
covMatrixTest <- covMatrixGaussian(x, y, theta = 2)
...
```{r }
likeFunction <- function(mu, theta ){
  # Function calculates likelihood value given parameter values
  covMatrix <- covMatrixGaussian(x, y, theta)
  detSigma <- det(covMatrix)
  invSigma <- solve(covMatrix)
  exponent <- t(y - mu) %*% invSigma %*% (y - mu)
  
  likelihood <- 1 / sqrt(2* pi* detSigma)* exp(exponent)
  
  return(likelihood)
}

nllFunction <- function(mu, theta ){
  # Function calculates likelihood value given parameter values
  covMatrix <- covMatrixGaussian(x, y, theta)
  detSigma <- det(covMatrix)
  invSigma <- solve(covMatrix)
...
library(stats4)
mleResults <- mle(nllFunction, start = list(mu = y / length(y), theta = 2))
```
