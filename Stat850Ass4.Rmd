...
```{r}
library(stats4)
mleResults <- mle(nllFunction, start = list(mu = mean(y), theta = 2), fixed = list(sigmaz = 1, sigmae = 1))
...
predCovMatrix <- function(fullX, theta){
  print(theta)
  nrow <- length(fullX)
  print(nrow)
  covMatrix <- matrix(0, nrow, nrow)
  
  # Fill the matrix with values
  for(i in 1:nrow){
    for(j in 1:nrow){
      covMatrix[i, j] <- covCalcGaussian(fullX[i], fullX[j], theta)
    }
  }
  return(covMatrix)
}
...
predData %>% ggplot(aes(xstar, preds)) + geom_point() +ggtitle("Plot of predictions along GP using MLE results") + geom_point(data = moreData, aes(x = xAll, y = yFull, shape = "*", col = "red")) + geom_point(data = actualData, aes(x, yact, col = "blue"))
```
